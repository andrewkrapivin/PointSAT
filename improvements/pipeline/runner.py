...
from improvements.pipeline.verified_cuts import load_verified_proof, supported_by_target
...
DEFAULTS = {
    ...
    "nonrealizability_proof_files": [],
    "output_storage": "legacy", "wall_time_limit": None, "quiet_progress": False,
}
_state = None
...
class PipelineDeadline(KeyboardInterrupt):
    """A requested wall budget expired; native workers still save checkpoints."""
...
def _process_job(job):
    ...
    try:
        if job["type"] == "SAT":
            ...
            if settings.get('nonrealizability_proof_files'):
                begin = time.perf_counter()
                if 'verified_proofs' not in _state:
                    _state['verified_proofs'] = [load_verified_proof(path, settings['n'])
                                                  for path in settings['nonrealizability_proof_files']]
                for proof in _state['verified_proofs']:
                    if supported_by_target(proof, job['orientations_file']):
                        timings['nonrealizability_proof_check'] = time.perf_counter()-begin
                        return dict(result, status='PROVED_NONREALIZABLE', proof_file=proof['path'],
                                    proof_sha256=proof['sha256'], proof_verified=proof['verified'],
    ...
    if _stop_requested:
        ...
        if result.get('status') == 'ERROR':
            ...
            result['interruption_detail'] = result.pop('error', 'Worker stopped')
    return result
...
def normalize_settings(settings):
    ...
    if settings['output_storage'] not in ('legacy', 'sqlite'):
        raise ValueError('output_storage must be legacy or sqlite')
    if settings['wall_time_limit'] is not None and (not isinstance(settings['wall_time_limit'], (int, float)) or
            not math.isfinite(settings['wall_time_limit']) or settings['wall_time_limit'] <= 0):
        raise ValueError('wall_time_limit must be a finite positive number')
    ...
    for key in ("localizer_extra_args", "sat_extra_args", "nonrealizability_proof_files"):
...
def run_pipeline(settings):
    ...
    compact = settings['output_storage'] == 'sqlite'
    if not compact and raw.exists():
    ...
    store, temporary = None, None
    if compact:
        from pointsat.store import RunStore
        store = RunStore(folder/'run.sqlite', create=True)
        store.set_meta('settings', settings)
        store.set_meta('state', 'running')
        # No automatic finalizer: exceptional exits retain uncommitted
        # coordinates for explicit recovery instead of deleting a witness.
        temporary = Path(tempfile.mkdtemp(prefix='.work-', dir=folder))
        scratch = temporary
    else:
    ...
    if store:
        store.set_meta('provenance', provenance)
    else:
    ...
    def record_result(result, log):
        if store:
            # Event, artifacts and accepted coordinates commit atomically.
            store.record(result, family=settings.get('problem_family'), register_solution=True)
        else:
    ...
    def reclaim_scratch():
        """Only delete our temporary files after their last queued/active use.

        Pending native jobs may create archive/radial files with their output
        name as a prefix; retain the whole namespace while the worker runs.
        Persisted event artifacts are already compressed in the database.
        """
        if not store:
            return
        referenced = set()
        active_prefixes = []
        for job in list(ready)+list(pending.values()):
            referenced.update(value for key, value in job.items()
                              if key.endswith('_file') and isinstance(value, str))
        for job in pending.values():
            if job.get('realization_file'):
                active_prefixes.append(job['realization_file'])
        for path in scratch.iterdir():
            name = str(path)
            if path.is_file() and name not in referenced and not any(name.startswith(prefix) for prefix in active_prefixes):
                path.unlink()
    ...
    try:
        with (nullcontext(None) if store else raw.open("x")) as log:
            while ready or pending:
                if settings['wall_time_limit'] is not None and time.perf_counter()-start >= settings['wall_time_limit']:
                    budget_exhausted = True
                    raise PipelineDeadline('Requested run wall budget expired')
                ...
                for future in done:
                    ...
                    record_result(result, log)
                    if not settings['quiet_progress']:
        ...
        with (nullcontext(None) if store else raw.open('a')) as log:
            while pending and time.perf_counter() < deadline:
                ...
                for future in done:
                    ...
                    try:
                        ...
                        record_result(dict(original, status='INTERRUPTED', error=str(exc)), log)
        for process in processes:
            if process.is_alive():
                process.terminate()
        for process in processes:
        ...
        if store:
            persisted = store.db.execute('SELECT count(*),coalesce(sum(solved),0),count(DISTINCT CASE WHEN solved THEN sample END) FROM events').fetchone()
            summary['persisted_events'] = persisted[0]
            summary['uncommitted_records'] = len(records)-persisted[0]
            summary['realized'] = persisted[1]
            summary['distinct_realized_samples'] = persisted[2]
            store.set_meta('summary', summary)
            store.set_meta('state', 'budget_exhausted' if budget_exhausted else 'interrupted' if interrupted else 'finished')
            if interrupted:
                store.set_meta('recovery_workspace', str(scratch.resolve()))
            store.close()
            if not interrupted:
