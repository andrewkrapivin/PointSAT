...
/* Exactly the same RNG draw and cumulative integer thresholds as above when
 * the caller already knows sum(WEIGHT_ADJUSTMENT*weights[i]+1). */
static inline int sample_proportional_known_total(const int *weights, int count,
                                                 int adjusted_total, rng_t *rng) {
    double r = rng_float(rng) * adjusted_total;
    int cumulative = 0;
    for (int i = 0; i < count; ++i) {
        cumulative += WEIGHT_ADJUSTMENT * weights[i] + 1;
